...
class CustomListWidget(QListWidget):
    ...
    def dropEvent(self, event):
        # Print MIME data formats for debugging
        print(f"Drop event in {self.objectName()}: MIME formats = {event.mimeData().formats()}")
        # Call the parent class's dropEvent to maintain default behavior
        super().dropEvent(event)

class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Drag and Drop with CustomListWidget")
        self.resize(400, 300)
