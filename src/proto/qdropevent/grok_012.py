...
class DraggableListWidget(QListWidget):
    ...
    def mousePressEvent(self, event):
        if event.button() == Qt.LeftButton:
            item = self.itemAt(event.position().toPoint())  # Updated to fix deprecation warning
            if item:
    ...
    def dropEvent(self, event: QDropEvent):
        if event.mimeData().hasText():
            ...
            # Calculate drop row
            drop_index = self.indexAt(event.position().toPoint())  # Updated to fix deprecation warning
