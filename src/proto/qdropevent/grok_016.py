...
class DraggableListWidget(QListWidget):
    ...
    def dragEnterEvent(self, event: QDragEnterEvent):
        # Accept any drop with text or source widget ID
        if event.mimeData().hasText() or event.mimeData().hasFormat("application/x-source-widget"):
            print(f"{self.name}: Drag entered, text: '{event.mimeData().text() if event.mimeData().hasText() else 'None'}', action: {event.proposedAction()}")
            event.acceptProposedAction()
        else:
            print(f"{self.name}: Drag rejected in dragEnterEvent (hasText: {event.mimeData().hasText()}, hasFormat: {event.mimeData().hasFormat('application/x-source-widget')}, action: {event.proposedAction()})")
            event.ignore()

    def dragMoveEvent(self, event: QDragMoveEvent):
        # Accept move events with text or source widget ID
        if event.mimeData().hasText() or event.mimeData().hasFormat("application/x-source-widget"):
            event.acceptProposedAction()
        else:
            print(f"{self.name}: Drag rejected in dragMoveEvent (hasText: {event.mimeData().hasText()}, hasFormat: {event.mimeData().hasFormat('application/x-source-widget')}, action: {event.proposedAction()})")
            event.ignore()
    ...
    def dropEvent(self, event: QDropEvent):
        if event.mimeData().hasText():
            ...
            print(f"{self.name}: Drop rejected in dropEvent (hasText: {event.mimeData().hasText()}, hasFormat: {event.mimeData().hasFormat('application/x-source-widget')}, action: {event.proposedAction()})")
            event.ignore()
...
class MainWindow(QWidget):
    def __init__(self, num_lists=3):
        ...
        layout.setContentsMargins(10, 10, 10, 10)  # Add margins for clarity
        layout.setSpacing(10)  # Space between lists
