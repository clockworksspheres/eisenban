...
CUSTOM_MIME = "application/x-kanban-task-text"
...
class KanbanColumn(QListWidget):
    ...
    def dropEvent(self, event):
        # Let Qt move the QListWidgetItem first
        super().dropEvent(event)

        # Now re-apply the custom CardItem using our serialized text
        if event.mimeData().hasFormat(CUSTOM_MIME):
            text_bytes = event.mimeData().data(CUSTOM_MIME)
            text = text_bytes.data().decode("utf-8")

            # The moved item should now be the currentItem (or last added)
            dropped_item = self.item(self.count() - 1) if self.count() > 0 else None
