...
class Environment(object):
            ...
            if cmd:
                ...
                for line in outputlines:
                    for vt in validtypes:
                        if re.search(vt, line.decode(), re.IGNORECASE):
                            self.systemtype = vt
    ...
    def getostype(self):
        ...
        @author D. Kennel
        """
        return self.operatingsystem.decode()
    ...
    def getosreportstring(self):
        ...
        @author D. Kennel
        """
        return self.osreportstring.decode()
    ...
    def discoveros(self):
        ...
        # method of getting version information
            ...
            build = build.strip()
            opsys = description.decode() + ' ' + release.decode() + ' ' + build.decode()
            self.osreportstring = opsys
    ...
    def guessnetwork(self):
        ...
        for line in netdata:
            ...
            match = re.search(macre, line.decode())
            if match is not None:
                # print 'Matched MAC address'
                macaddress = match.group()
            if re.search(ipaddress, line.decode()):
    ...
    def getdefaultip(self):
        ...
        if os.path.exists('/usr/bin/lsb_release'):
            ...
            try:
                ...
                return ipaddr
            for line in routedata:
                if re.search('gateway:', line.decode()):
                    line = line.split()
                    try:
                        gateway = line[1]
                    except IndexError:
    ...
    def getallips(self):
        ...
        try:
            ...
            raise
        for line in ifdata:
            if re.search('inet addr:', line.decode()):
                try:
                    line = line.split()
                    addr = line[1]
                    addr = addr.split(':')
                    ...
                    continue
            elif re.search('inet ', line.decode()):
                try:
    ...
    def get_system_serial_number(self):
        ...
        if DMI and self.euid == 0:
            ...
            for line in cmd3output:
                if re.search('Serial Number (system):', line.decode()):
                    line = line.split(':')
                    try:
                        systemserial = line[1]
                    except(IndexError, KeyError):
