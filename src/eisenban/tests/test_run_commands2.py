...
from eisenban.lib.run_commands import (
    NotACyLoggerError,
...
# from eisenban.lib.loggers import CyLogger, MockLogger, LogPriority as lp
...
from eisenban.lib.loggers import MockLogger as CyLogger
...
class TestRunWithInit(unittest.TestCase):
    ...
    def test_defaults_use_mock_logger_when_no_logger_supplied(self):
        with patch('eisenban.lib.run_commands.MockLogger') as MockLogger:
            rw = RunWith()
...
class TestCommunicate(unittest.TestCase):
    ...
    @patch("eisenban.lib.run_commands.Popen")
...
class TestTimeoutMethod(unittest.TestCase):
    ...
    @patch("eisenban.lib.run_commands.threading.Timer")
    @patch("eisenban.lib.run_commands.Popen")
...
class TestRunWithSudoSubprocess(unittest.TestCase):
    ...
    @patch("eisenban.lib.run_commands.os.set_blocking")
    @patch("eisenban.lib.run_commands.select.select", return_value=([], [], []))
    @patch("eisenban.lib.run_commands.subprocess.Popen")
    ...
    @patch("eisenban.lib.run_commands.os.set_blocking")
    @patch("eisenban.lib.run_commands.select.select")
    @patch("eisenban.lib.run_commands.subprocess.Popen")
...
class TestRunMyThreadCommand(unittest.TestCase):
    ...
    @patch("eisenban.lib.run_commands.RunThread")
