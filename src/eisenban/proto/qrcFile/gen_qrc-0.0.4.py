...
def generate_qrc_file(output_path, icon_dir):
    ...
    qresource.set("prefix", "/img")  # You can change the prefix as needed

    img_dir = "resources/img"  # Path to the directory containing icons
    img_extensions = [".png", ".jpg", ".jpeg", ".gif", ".svg"]  # List of icon file extensions
    img_files = []
    for img in img_extensions:
        # Use glob to find files with the specified extension
        for file_path in glob.glob(f"{img_dir}/**/*{img}", recursive=True):
    ...
    qresource.set("prefix", "/font")  # You can change the prefix as needed
    font_dir = "resources/font"  # Path to the directory containing icons
    font_extensions = [".ttf"]  # List of font file extensions
    font_files = []
    for font in font_extensions:
        # Use glob to find files with the specified extension
        for file_path in glob.glob(f"{font_dir}/**/*{font}", recursive=True):
    ...
    icon_dir = "resources/icons"  # Path to the directory containing icons
    icon_extensions = [".ico", ".icns"]  # List of icon file extensions
    icon_files = []
    for icon in icon_extensions:
        # Use glob to find files with the specified extension
        for file_path in glob.glob(f"{icon_dir}/**/*{icon}", recursive=True):
