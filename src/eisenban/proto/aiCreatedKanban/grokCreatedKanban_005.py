...
class KanbanColumn(QListWidget):
    def __init__(self, title="New Column"):
        ...
        self.setDragDropMode(QListWidget.DragDropMode.InternalMove)  # Handles moves automatically
        self.setDefaultDropAction(Qt.DropAction.MoveAction)
        self.setSelectionMode(QListWidget.SelectionMode.SingleSelection)

    def mimeTypes(self):
        return [MIME_TYPE] + super().mimeTypes()
    ...
    def mimeData(self, items):
        ...
        # Serialize the text of the first selected item
        item = items[0]
        widget = self.itemWidget(item)
        if widget and isinstance(widget, CardItem):
            data.append(widget.label.text().encode('utf-8'))

        mime_data.setData(MIME_TYPE, data)
        # Also set Qt's default data so InternalMove works
        super_mime = super().mimeData(items)
        for fmt in super_mime.formats():
            mime_data.setData(fmt, super_mime.data(fmt))
    ...
    def dropEvent(self, event):
        ...
        # Let Qt handle the actual item move/copy first
        super().dropEvent(event)

        # Now re-apply the custom widget using our serialized text
        if event.mimeData().hasFormat(MIME_TYPE):
            data = event.mimeData().data(MIME_TYPE)
            if data:
                text = bytes(data).decode('utf-8')

                # Find the newly dropped item (usually the current one after drop)
