...
CUSTOM_MIME = "application/x-kanban-card"
...
class KanbanColumn(QListWidget):
    def __init__(self, title="New Column"):
        ...
        self.setDragDropMode(QListWidget.DragDropMode.DragDrop)  # Allows external drops
        self.setDefaultDropAction(Qt.DropAction.MoveAction)
        self.setSelectionMode(QListWidget.SelectionMode.SingleSelection)

        # Connect to rowsInserted to restore the card after any insert/move
        self.model().rowsInserted.connect(self.restore_card_after_insert)
    ...
    def dropEvent(self, event):
        # Let Qt handle the item move/copy first
    ...
    def restore_card_after_insert(self, parent, first, last):
        """Re-apply CardItem to newly inserted/moved items if missing"""
        if not self.mimeData(self.selectedItems()).hasFormat(CUSTOM_MIME):
            return  # No custom data available

        text_bytes = self.mimeData(self.selectedItems()).data(CUSTOM_MIME)
        if not text_bytes:
            return
        text = text_bytes.data().decode("utf-8")
...
class KanbanBoard(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("PySide6 Kanban Board - Smooth Drag & Drop")
