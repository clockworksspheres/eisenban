...
class KanbanColumn(QListWidget):
    def __init__(self, title="New Column"):
        ...
        # Connect to rowsInserted to re-apply card after move/drop
        self.model().rowsInserted.connect(self.on_rows_inserted)

        # Store text temporarily during drag (needed for inter-list moves)
        self._drag_text = None

    def startDrag(self, supportedActions):
        # Save the text of the dragged card
        item = self.currentItem()
        widget = self.itemWidget(item)
        if widget and isinstance(widget, CardItem):
            self._drag_text = widget.label.text()
        super().startDrag(supportedActions)

    def on_rows_inserted(self, parent, first, last):
        # After Qt inserts/moves the item, re-apply the CardItem
        for row in range(first, last + 1):
            item = self.item(row)
            current_widget = self.itemWidget(item)
            if current_widget is None:
                # Use saved text if available (inter-list move), otherwise fallback
                text = self._drag_text or "New Task"
                card = CardItem(text)
                item.setSizeHint(card.sizeHint())
                self.setItemWidget(item, card)
        self._drag_text = None  # Clear after use
...
class KanbanBoard(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("PySide6 Kanban Board - Fully Working Drag & Drop")
